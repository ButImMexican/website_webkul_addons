...
class WebsiteMessageWizard(models.TransientModel):
    ...
    def update_latest_record(self):
        ...
        active_website_record = active_model.browse(active_id)
        if not active_website_record.website_id:
            is_active_record = active_model.search([('is_active','=',True)])
        else:
            is_active_record = active_model.search([('is_active','=',True),('website_id','=',active_website_record.website_id.id)])
